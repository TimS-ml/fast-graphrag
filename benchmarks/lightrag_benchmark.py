"""Benchmarking script for LightRAG knowledge graph implementation.

This module evaluates LightRAG, an alternative graph-based RAG implementation,
against the same benchmark datasets used for FastGraphRAG. It provides a
comparison point to assess different knowledge graph architectures.

LightRAG uses a different approach to entity extraction and graph construction,
providing useful baseline measurements for understanding design trade-offs.

Pipeline:
    1. Load benchmark datasets (2wikimultihopqa, hotpotqa)
    2. Build knowledge graph using LightRAG
...
This comparison helps evaluate:
    - Differences in knowledge graph quality
    - Query mode performance (local vs global vs hybrid)
    - Answer generation accuracy
    - Retrieval latency differences
    - Entity extraction approach differences

Key operations:
    - Dataset loading and deduplication
    - LightRAG graph construction using GPT-4o-mini
    - Asynchronous query execution in different modes
    - Retrieval metric computation
    - Per-dataset analysis of performance

Usage:
    python lightrag_benchmark.py -d <dataset> -n <subset_size> [--mode <mode>] [-c] [-b] [-s]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="LightRAG Comparison Benchmark")
    ...
    parser.add_argument("-c", "--create", action="store_true", help="Build LightRAG graph from corpus.")
    ...
    parser.add_argument("--mode", default="local", help="LightRAG query mode (local, global, hybrid).")
    ...
    # Phase 1: Build LightRAG knowledge graph from corpus passages
    # Uses different entity extraction and graph construction approach than FastGraphRAG
    if args.create:
        ...
        # Insert passages for knowledge graph construction
        grag.insert([f"{title}: {corpus}" for _, (title, corpus) in tuple(corpus.items())])

    # Phase 2: Query the LightRAG graph and evaluate retrieval performance
    # Tests different query modes: local (entity focus), global (big picture), hybrid
    if args.benchmark:
        ...
        async def _query_task(query: Query, mode: str) -> Dict[str, Any]:
            """Execute a single query using LightRAG.
            ...
            Note:
                Parses CSV-formatted source citations from LightRAG output to extract
                document titles. Limits to top-8 results for consistent comparison.
            """
            # Query LightRAG in specified mode without full answer generation
            answer = await grag.aquery(
                query.question, QueryParam(mode=mode, only_need_context=True, max_token_for_text_unit=9000)
            )
            # Parse LightRAG's CSV-formatted sources to extract document titles
            chunks = [
    ...
    # Evaluates how well LightRAG's knowledge graph retrieves ground truth evidence
